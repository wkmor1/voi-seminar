...
Ask questions during talk if don't make sense
...
- .grey[.font120[Value of information for mangement of box-ironbark woodlands]]
...
VOI almost always an expected value. Value from decision expected. sum of the outcomes weighted by their probility of happening
...
EVWPI is only slighly different to EVWOI
...
That is unlike perfect information, it is not the EVI of resolving all uncertaity it is the value of only resolve a bit
...
Graphically that idea looks this. this graph also higlights a point will come back to later.

VOI in general, at least theoretically, has diminising returns.
...
--
- .font130[EVPI is the upper bound, EVSI is the genral form and has diminishing returns]
...
For the sake of argument lets replace sample-size with time, assuming over time we accumulate information and the cummulative value of information is increasing towards perfect knowledge
...
It tries to maxmise the average proportion of value remaining when removing a grid cell in reverse order
...
imporantly though we still had to calculate it to work out it wasn't the max
...
Not neccessarly the best way of modelling species distributions but certainly a very common method used in conservation planning
...
EVWOI computationally hard cos you need to cross assess each 1000 plans againts each 1000 bs species mapset = 1 million performance assessments.
...
I've artifically reduced the available points so to ensure there is a measureable level of uncertainty.  

Covariates used: mean solar radiation, annual mean temperature, annual preciptation, COV of precipitation, soil fertility and topographic wetness.
...
With those inputs the planning algorithm ranks sites in the west low and sites in the east high. with some low ranking cells around new castle and also along the coast. havent checked but this probably a pretty good map of existing and proposed coal mines :(
...
So if you ignore uncertainty and you could put reserves whereever you want over 20% of the region you'd put them here.

Ignoring a whole bunch of caveats like connnectivity and the appropriateness of the objective function and missing species and other considerations.
...
--
.font120[Species with orginal population of 10,000 expect to protect .purple[40] more with reserve network than would otherwise]
...
Haven't tested it but could be a usefull heuristic and has come up in some of my other VOI work
...
When the probabilty of failure is below they would chose to restore
...
A risk neutral decision maker exibits 1 to 1 relationship they percieve value as value and only care about maxmising there expected value when making a decision

someone who is risk-averse, percieves diminishing returns with value and importatly the percieve high value has having comparably lower utility compared to a risk neutralists
